...
import com.verNANDo57.rulebook_educational.extradata.BuildConfig;
...
public class PdfiumCore {
    ...
    static {
        try {
            ...
            Log.e(LOG_TAG, "Native libraries failed to load - " + e);
        }
    }
    ...
    public PdfiumCore(Context ctx) {
        ...
        Log.d(LOG_TAG, "Starting PdfiumAndroid " + BuildConfig.VERSION_NAME);
    }
    ...
    public void openPage(PdfDocument doc, int pageIndex) {
        long pagePtr;
        synchronized (lock) {
    ...
    public void renderPage(PdfDocument doc, Surface surface, int pageIndex,
        ...
        synchronized (lock) {
            try {
                ...
                Log.e(LOG_TAG, "mContext may be null");
                e.printStackTrace();
            } catch (Exception e) {
                Log.e(LOG_TAG, "Exception throw from native");
                e.printStackTrace();
            }
        }
    }
    ...
    public PdfDocument.Meta getDocumentMeta(PdfDocument doc, Context context) {
        synchronized (lock) {
            PdfDocument.Meta meta = new PdfDocument.Meta();
            meta.title = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_title));
            meta.author = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_author));
            meta.subject = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_subject));
            meta.keywords = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_keywords));
            meta.creator = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_creator));
            meta.producer = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_producer));
            meta.creationDate = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_creationDate));
            meta.modDate = nativeGetDocumentMetaText(doc.mNativeDocPtr, context.getString(R.string.app_pdf_modDate));
