 ...
 *
 * Once you set a desired locale using [setLocale] method, CustomLocaleEngine will enforce your application
 * to provide correctly localized data via [Resources] class.
 */
class CustomLocaleEngine private constructor(
     ...
     * [CustomLocaleEngine] is not responsible for that.
     *
    ...
    internal fun initialize(application: Application) {
        application.registerActivityLifecycleCallbacks(
            CustomLocaleEngineActivityLifecycleCallbacks {
                applyForActivity(it)
            }
        )
        application.registerComponentCallbacks(
            CustomLocaleEngineApplicationCallbacks {
    ...
    companion object {
        ...
        private lateinit var instance: CustomLocaleEngine

        /**
         * Returns the global instance of [CustomLocaleEngine] created via init method.
         *
         * @throws IllegalStateException if it was not initialized properly.
         */
        @JvmStatic
        fun getInstance(): CustomLocaleEngine {
         ...
         */
        @JvmStatic
        fun init(application: Application, defaultLanguage: String): CustomLocaleEngine {
        ...
        @JvmStatic
        @JvmOverloads
        fun init(application: Application, defaultLocale: Locale = Locale.getDefault()): CustomLocaleEngine {
         ...
         *
         * This method must be called before any calls to [CustomLocaleEngine] and may only be called once.
         */
        @JvmStatic
        fun init(application: Application, store: LocaleStore): CustomLocaleEngine {
            check(!::instance.isInitialized) { "Already initialized" }
            val lingver = CustomLocaleEngine(store, UpdateLocaleDelegate())
            lingver.initialize(application)
            instance = lingver
            return lingver
        }

        internal fun createInstance(store: LocaleStore, delegate: UpdateLocaleDelegate): CustomLocaleEngine {
            return CustomLocaleEngine(store, delegate)
        }
    }
}
